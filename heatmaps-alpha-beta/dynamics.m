...
function dynamics(alpha_eta, beta_eta, wave)
...
setappdata(0, 'alpha_var', alpha_eta) % load fms value
                          % in parameter file fms= getappdata(0,'fms_var')


setappdata(0, 'beta_var', beta_eta) % load fmas value
...
%---------------Solve ODE
[T, Y] = ode15s(@(t,y)rhs(t, y, fMan, fMn, wave), Tspan, IC); % Solve ODE
