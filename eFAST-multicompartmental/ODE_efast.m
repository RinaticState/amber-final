function dy = ODE_efast(t,y,fMat,fMt, wave)
...
dy(4) = muBSE*(Qblood/Qspleen)*Rb - muSB_tD.*Rs + bR*tDs.*Ems + aR*((Tnaive/Qspleen) - Rs)...
    - muR*Es.*Rs - ram*Rs + bP*(tDs.*Rs)./(thetaD + tDs); %muR = muE, recycled value
...
dy(20) = Jnew*(Qblood/Qpanc) + (k+b)*Ma-c*M - fMt*M.*Ba - fMt*M.*Bn - e1*M.*(M+Ma);
